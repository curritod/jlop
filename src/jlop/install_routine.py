...
def install_mpl_themes(overwrite=False):
    """
    Adapted from
    ...
    if answer.lower() in ["y","yes"]:
        ...
        for file in glob(os.path.join(dir_src_fonts, '*.[ot]tf')):
            ...
            dst_file = os.path.join(dir_dest_fonts, os.path.basename(file))
            if not os.path.exists(dst_file):
                print(f'Adding font "{os.path.basename(file)}".')
                shutil.copy(file, dir_dest_fonts)
            elif overwrite==True:
                print('File', dst_file, 'already exists. Overwriting it.')
                os.remove(dst_file)
                shutil.copy(file, dir_dest_fonts)
            else:
                print('File', dst_file, 'already exists')
                answer = input("Overwrite? [y/N] ")
                if answer.lower() in ["y","yes"]:
                    print(f'Overwriting font "{os.path.basename(file)}".')
                    os.remove(dst_file)
                    shutil.copy(file, dir_dest_fonts)
                else:
                    print('no change to file', dst_file)
        ...
        for file in glob(os.path.join(dir_src_styles, '*.mplstyle')):
            ...
            dst_file = os.path.join(dir_dest_styles, os.path.basename(file))
